...
const List = ({ webId, gamePath, sendNotification }: Props) => {
  ...
  const resignedGame = async ({ url, documentUrl, status, actor }) => {
    if (status !== GameStatus.FINISHED) {
      ...
      if (!appPath) {
        ...
        appPath = await getTTTAppStorage(actor.webId);
      }
  ...
  const init = useCallback(async () => {
    setIsLoading(true);
    const getWebIdAppStorage = storageHelper.getAppStorage(process.env.REACT_APP_TICTAC_PATH)
    appPath = await getWebIdAppStorage(webId);
